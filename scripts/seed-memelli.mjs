  ...
  await sql.begin(async (tx) => {
    ...
    for (const lt of leadTypes) {
      const [row] = await tx`
        ...
        RETURNING id, name
      `;
      if (row) insertedLeadTypes.push(row);
    }
    console.log(`Inserted ${insertedLeadTypes.length} lead types`);
    ...
    const [scoringModel] = await tx`
      ...
      RETURNING id
    `;
    const smId = scoringModel?.id || (await tx`SELECT id FROM tenant_scoring_models WHERE tenant_id = ${tenant.id} AND name = 'default'`)[0].id;
    ...
    for (const cat of kwCategories) {
      const [catRow] = await tx`
        ...
        RETURNING id
      `;
      const catId = catRow?.id || (await tx`SELECT id FROM keyword_categories WHERE tenant_id = ${tenant.id} AND name = ${cat.name}`)[0].id;
